...
__host__ void nbodyTimer(void * kernelInfo);
...
__device__ int getIndexForRank(const int rank, const int size, const int n);
...
int main(int argc, char ** argv)
{
  ...
  dcgn::initAll(&argc, &argv, 0, gpus, 1, 0, -1);
  ...
  kinfo.bodies = new Body[kinfo.n];
  fread(kinfo.bodies, sizeof(Body) * kinfo.n, 1, fp);
  fclose(fp);
  sscanf(argv[3], "%d", &kinfo.timeSteps);
  sscanf(argv[4], "%f", &kinfo.timeDelta);

  dcgn::launchGPUKernel(0, nbodyWrapper,  nbodyTimer, reinterpret_cast<void * >(&kinfo), gs, bs);
...
__host__ void nbodyWrapper(void * kernelInfo, const dcgn::GPUInitRequest libParam, const uint3 & gridSize, const uint3 & blockSize, const int sharedMemSize, cudaStream_t * const stream)
{
  ...
  cudaMemcpy(gpuBodies, kinfo->bodies, sizeof(Body) * kinfo->n, cudaMemcpyHostToDevice);                                                              CHECK_ERR();
...
__global__ void nbody(dcgn::GPUInitRequest libParam, Body * gpuBodies, Body * gpuUpdatedBodies, volatile int * sbarr, const int size, const int timeSteps, const float timeDelta)
{
  ...
  for (int i = 0; i < timeSteps; ++i)
  {
    ...
    if (threadIdx.x == 0 && blockIdx.x == 0)
    {
      ...
      {
        const int start = getIndexForRank(j,     dcgn::gpu::getSize(), size);
        const int end   = getIndexForRank(j + 1, dcgn::gpu::getSize(), size);
